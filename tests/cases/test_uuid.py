...
def test_uuid_collision(session: Session):
    ...
    case_1 = Case(id=case_id, case_type=CaseTypes.CLA)
    case_2 = Case(id=case_id, case_type=CaseTypes.CLA)
    session.add(case_1)
    session.add(case_2)
...
def test_existing_uuid_collision(session: Session):
    ...
    session.commit()
    new_case = Case(id=original_id, case_type=CaseTypes.CLA)
...
def test_invalid_uuid_int(session: Session):
    with pytest.raises(ValidationError):
        Case(id=1, case_type=CaseTypes.CLA)


def test_invalid_uuid_str(session: Session):
    with pytest.raises(ValidationError):
        Case(id="string", case_type=CaseTypes.CLA)
